...
{
    public class ACreateCustomerAPIControllerTests
    {
        [Fact(DisplayName = "Create the CustomerAPIController @create-the-homecontroller")]
        public void CreateCustomerAPIControllerTest()
        {
            // Get appropriate path to file for the current operating system
            var filePath = ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + "APIStarterSolution" + Path.DirectorySeparatorChar + "Controllers" + Path.DirectorySeparatorChar + "CustomerAPIController.cs";
            // Assert Index.cshtml is in the Views/Home folder
            Assert.True(File.Exists(filePath), "`CustomerAPIController.cs` was not found in the `Controllers` folder.");

            var controllerType = TestHelpers.GetUserType("APIStarterSolution.Controllers.CustomerAPIController");

            Assert.True(controllerType != null, "`CustomerAPIController.cs` was found, but does not appear to contain a `public` `CustomerAPIController` class.");
            Assert.True(controllerType.BaseType == typeof(ControllerBase), "`CustomerAPIController` was found, but is not inheriting the `ControllerBase` class. (you will need a using directive for `Microsoft.AspNetCore.Mvc`)");
        }
